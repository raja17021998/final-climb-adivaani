        ...
        for hub in hubs:
            for src, tgt in [(hub, tribal), (tribal, hub)]:
                ...
                for _, row in df.iterrows():
                    ...
                    preds.append(pred)
                    refs.append([str(row[tgt_col])])

                bleu_score = bleu.compute(predictions=preds, references=refs)["bleu"] * 100
                ...
                out["BLEU Score (100)"] = bleu_score
                out["CHRF++ Score (100)"] = chrf_score
                out.to_csv(f"eval_{tribal}_{src}_{tgt}.csv", index=False)
