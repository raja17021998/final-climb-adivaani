...
def evaluate_file(lang, direction, file):
    ...
    iterator = tqdm(
        ...
        total=len(df),
        desc=f"Eval {lang}-{direction}"
    )

    for s, t in iterator:
        p = infer(s, src, tgt)

        bleu_score = bleu.compute(predictions=[p], references=[[t]])["bleu"] * 100
        chrf_score = chrf.compute(predictions=[p], references=[[t]])["score"]

        rows.append({
            f"Source Language {src_col}": s,
            f"Actual {tgt_col}": t,
            f"Predicted {tgt_col}": p,
            "BLEU Score (100)": bleu_score,
            "CHRF++ Score (100)": chrf_score
        })
...
def main():
    ...
    parser.add_argument("--file", type=str, default=None, help="optional path to eval CSV file")
    ...
    for direction in active_dirs:
        evaluate_file(lang, direction, test_file)
